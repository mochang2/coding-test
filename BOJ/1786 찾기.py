...
후기: 문자열 알고리즘이다. 이 문제는 문자열 예시에 공백도 존재하므로 input_().strip()으로 입력을 받으면 안 된다.
추가적으로 입력받을 때 replace가 아니라 rstrip을 사용하면 입력값을 순회를 할 필요가 없기 때문에 더 빠르다.
KMP와 Rabin Karp를 이용해서 풀었다.

찾아야 하는 문자열을 pattern, 찾는 위치를 text라고 보통 명명한다.
일반적인 완전 탐색은 O(len(pattern) * len(text))의 시간 복잡도를 가진다.
...
Rabin Karp는 해시를 이용한 알고리즘이다.
각 문자열을 아스키 값으로 변경한 뒤 해당 위치에 따라 base의 제곱수를 곱한 뒤 모두 더한다.
파이썬에는 오버플로우가 없으므로 mod 연산을 안 해줘도 되지만 일반적인 알고리즘을 구현한다 생각하여 mod 연산을 추가했다.
더 자세한 설명은 https://www.youtube.com/watch?v=kJJQJDsjXc8 를 이용하자.

KMP 알고리즘을 이용한 첫 번째 코드는 실패했다.
KMP_table 구하는 공식만 보고 구했다가 문자열을 발견한 뒤 target_index를 재설정하는 부분에서 비효율성이 발생했다.
...
# 첫 번째 KMP
# 오답. 시간 초과
# 문자열을 발견한 뒤 target_index를 재설정하는 비효율성 발생
...
def getStartIndexes(text, pattern):
    ...
    for start_index in range(text_length - pattern_length + 1):
        sub_text = text[start_index:start_index + pattern_length] # 비효율성 
        """
...
## 실무에서는 일어날 수 있으므로 해당 문제에서만 적용 가능
## base를 global로 선언했어도 좋았을 것 같음
...
def calculateHash(string, powers):
    ...
    return mod(hash_value) # mod로 감싸주지 않았어서 많이 틀렸었음
